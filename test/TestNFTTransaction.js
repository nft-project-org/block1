...
contract("NFTTransaction", function (accounts) {
  ...
  describe("verifyTokenOwner", function () {
    it("should return the owner of a token", async function () {
      const tokenId = new BN("123456789")
      await nftTransaction.mint(accounts[0], tokenId)
      expect(await nftTransaction.verifyTokenOwner(tokenId)).to.equal(
        accounts[0]
      )
    })
  })
  ...
  it("lists a token for sale", async () => {
    ...
    await instance.mint(accounts[0], tokenId)
  ...
  it("buys a token", async () => {
    ...
    const owner = await instance.verifyTokenOwner(1)
